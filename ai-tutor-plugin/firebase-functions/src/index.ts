...
// Interface for embeddings document
interface EmbeddingsDocument {
  metadata: {
    title: string;
    source_file: string;
    processed_at: string;
...
// Interface for function call data
 ...
 */
function generateFallbackResponse(userQuery: string, contextChunks: Chunk[]): string {
  ...
  if (queryLower.includes('what is') || queryLower.includes('define') || queryLower.includes('definition')) {
    ...
    const definitionChunks = contextChunks.filter(chunk => 
  ...
  if (queryLower.includes('axiom') || queryLower.includes('rule') || queryLower.includes('law')) {
    ...
    const axiomChunks = contextChunks.filter(chunk => 
      chunk.content.toLowerCase().includes('axiom') || 
  ...
  if (queryLower.includes('conditional') || queryLower.includes('given')) {
    ...
    const conditionalChunks = contextChunks.filter(chunk => 
      chunk.content.toLowerCase().includes('conditional') || 
...
export const ragQuery = functions
  ...
  .https.onCall(async (data: RAGQueryData, context: functions.https.CallableContext) => {
  try {
    // Validate input
    ...
    if (!TOPIC_PATHS[topic]) {
      throw new functions.https.HttpsError(
        'invalid-argument',
        `Invalid topic: ${topic}. Available topics: ${Object.keys(TOPIC_PATHS).join(', ')}`
      );
    }
    ...
    if (!embeddingsDoc.exists) {
      throw new functions.https.HttpsError(
        'not-found',
        'Embeddings document not found at the specified path'
      );
    }
    ...
    if (docData.embeddings) {
      ...
      throw new functions.https.HttpsError(
        'not-found',
        `Embeddings data not found in document. Available fields: ${Object.keys(docData).join(', ')}`
      );
    }
    ...
    if (!embeddingsData.chunks || !Array.isArray(embeddingsData.chunks)) {
      ...
      throw new functions.https.HttpsError(
        'not-found',
        `Chunks data is not an array. Type: ${typeof embeddingsData.chunks}, Value: ${JSON.stringify(embeddingsData.chunks)}`
      );
    }
    ...
    let contextChunks: Chunk[] = [];
    let topChunks: SimilarityResult[] = [];
    ...
    return {
      ...
      context: {
        ...
        similarities: topK > 0 && topChunks.length > 0 ? topChunks.map((result) => ({
          ...
          similarity: result.similarity, // Use actual calculated similarity
    ...
    throw new functions.https.HttpsError(
      'internal',
      `An internal error occurred while processing your request: ${errorMessage}`
    );
  }
});
 ...
 */
export const testEmbeddings = functions
  .runWith({
    timeoutSeconds: 60, // 1 minute
    memory: '256MB'
  })
  .https.onCall(async (data: { topic?: string }, context: functions.https.CallableContext) => {
  try {
    const topic = data.topic || 'probability';
    ...
    throw new functions.https.HttpsError(
      'internal',
      `An internal error occurred while testing embeddings: ${errorMessage}`
    );
  }
});
